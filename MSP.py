...
#Copyright (C) 2019 John Alejandro González
...
class MultiWii(object):
	...
	__VERSION__ = "0.0.15"
	#FORKED FORM JONATAH DEAN(ke4ukz@gmx.com) MSPIO REPO
	#THIS IS A MSP CLASS FOR DATA CONTROL ON PYTHON 3
	__AUTHOR__ = "John Alejandro González (johnalejandrog.g4@gmail.com)"
	#Instance variables:
...
# connection methods #################################################################################
	...
	def connect(self, portName, baudRate):
		...
		return self._serial.is_open and self._monitorThread.isAlive()
...
# set* methods #################################################################################
	...
	def setMotors(self,motors):
		...
		Notes:
			Make sure you got the props out
		"""
		data = bytearray()
		for i in range(0,7):
			r = self._fromUInt16(min(2000, max(1000, motors[i])))
			data.append(r[0])
			data.append(r[1])
		#return data
		return self._sendAndWait(self._MSPCOMMANDS.MSP_SET_MOTOR, data)